...
contract Exchange is Operatable, Migratable {
    ...
    function addTokenPair(
        ...
        TokenData memory tokenDataItem =
            TokenData({oracleTicker: oracleTicker, smelter: ISmelter(smelter)});
    ...
    function getUnitPriceAPerB(address tokenA, address tokenB)
        public
        view
        returns (uint256)
    {
    ...
    function mintableToken(address token) public view returns (address) {
        return
            isMintable(token)
                ? address(mintableTokens[address(unmintableTokens[token])])
                : address(mintableTokens[token]);
    }
    ...
    function swap(
        ...
        require(
            mintableFromToken != address(0),
            "Exchange: fromToken isn't managed"
        );
        require(
            mintableToToken != address(0),
            "Exchange: toToken isn't managed"
        );
        ...
        require(
            address(mintableFromSmelter) != address(0),
            "Exchange: FromToken is not managed"
        );
        require(
            address(mintableToSmelter) != address(0),
            "Exchange: ToToken is not managed"
        );
        ...
        if (isMintable(fromToken)) {
            mintableFromSmelter.burnSyntheticFrom(
                fromToken,
                msg.sender,
                amountIn
            );
        } else {
            SyntheticToken(fromToken).transferFrom(
                msg.sender,
                address(this),
                amountIn
            );
