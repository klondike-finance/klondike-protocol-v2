...
def distributor(accounts, chain, ve_boardroom, ve_token, token):
...
@given(
    st_amount=strategy("decimal[10]", min_value=1,
                       max_value=100, places=4, unique=True),
    st_locktime=strategy("uint256[10]", min_value=1,
                         max_value=52, unique=True),
    st_sleep=strategy("uint256[10]", min_value=1, max_value=30, unique=True),
)
def test_checkpoint_total_supply(
    accounts, chain, distributor, ve_token, st_amount, st_locktime, st_sleep
):
    final_lock = 0
    for i in range(10):
        ...
        ve_token.create_lock(
            int(st_amount[i] * 10 ** 18), lock_time, {"from": accounts[i]})
