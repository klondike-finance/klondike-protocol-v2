...
def test_deposited_after(web3, chain, accounts, ve_token, ve_boardroom, coin_a, token, fn_isolation):
    ...
    ve_token.create_lock(amount, chain[-1].timestamp + 3 * WEEK, {"from": alice})
    chain.sleep(2 * WEEK)
...
def test_deposited_during(web3, chain, accounts, ve_token, ve_boardroom, coin_a, token, fn_isolation):
    alice, bob = accounts[0:2]
    amount = 1000 * 10 ** 18
    ...
    chain.sleep(WEEK)
    ve_token.create_lock(amount, chain[-1].timestamp + 8 * WEEK, {"from": alice})
    chain.sleep(WEEK)
    ve_boardroom = ve_boardroom()
    ve_boardroom.add_token(coin_a, chain.time())
...
def test_deposited_before(web3, chain, accounts, ve_token, ve_boardroom, coin_a, token, fn_isolation):
    alice, bob = accounts[0:2]
    amount = 1000 * 10 ** 18
...
def test_deposited_twice(web3, chain, accounts, ve_token, ve_boardroom, coin_a, token, fn_isolation):
    alice, bob = accounts[0:2]
    amount = 1000 * 10 ** 18
    ...
    ve_token.create_lock(amount, chain[-1].timestamp + 4 * WEEK, {"from": alice})
    chain.sleep(WEEK)
    chain.mine()
    start_time = int(chain.time())
    chain.sleep(WEEK * 3)
...
def test_deposited_parallel(web3, chain, accounts, ve_token, ve_boardroom, coin_a, token, fn_isolation):
