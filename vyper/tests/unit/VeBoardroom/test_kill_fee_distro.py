...
def test_killing_xfers_tokens(ve_boardroom, accounts, coin_a):
...
def test_multi_kill_token_xfer(ve_boardroom, accounts, coin_a):
...
@pytest.mark.parametrize("idx", range(1, 3))
def test_cannot_claim_after_killed(ve_boardroom, accounts, idx, coin_a):
    ve_boardroom.kill_me({"from": accounts[0]})
