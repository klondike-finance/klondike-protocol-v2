...
describe("LockPool", () => {
  ...
  beforeEach(async () => {
    ...
    jedi = await SyntheticToken.deploy("DROID", "DROID", 18);
    droid = await SyntheticToken.deploy("JEDI", "JEDI", 18);
    await jedi.mint(op.address, ETH.mul(100));
    await droid.mint(op.address, ETH.mul(100));
    lockPool = await LockPool.deploy(jedi.address, droid.address, await now());
    await droid.transferOperator(lockPool.address);
    await droid.transferOwnership(lockPool.address);
    await jedi.approve(lockPool.address, ethers.constants.MaxUint256);
  ...
  describe("#constructor", () => {
    it("creates a new LockPool", async () => {
      await expect(LockPool.deploy(jedi.address, droid.address, await now())).to
        .not.be.reverted;
    });
  });
    ...
    describe("all the conditions are good", () => {
      it("locks the tokens and returns the reward", async () => {
        ...
        await jedi.transfer(
          ...
          (await jedi.balanceOf(op.address)).sub(amount * 7)
        );
        ...
        expect(await droid.balanceOf(op.address)).to.eq(
        ...
        expect(await jedi.balanceOf(op.address)).to.eq(amount);
      });
    });
  ...
  describe("#unlock", () => {
    describe("when all prerequisites are met", () => {
      it("transfers all available tokens (1)", async () => {
        await jedi.transfer(
          "0x0000000000000000000000000000000000000001",
          (await jedi.balanceOf(op.address)).sub(30000)
        );
        ...
        expect(await jedi.balanceOf(op.address)).to.eq(10000);
        ...
        expect(await jedi.balanceOf(op.address)).to.eq(0);

        await fastForwardAndMine(ethers.provider, 30);
        // 30 * 86400 + 1002
        await expect(lockPool.unlock()).to.not.be.reverted;
        expect(await jedi.balanceOf(op.address)).to.eq(20000);
    ...
    it("transfers all available tokens (2)", async () => {
      await jedi.transfer(
        "0x0000000000000000000000000000000000000001",
        (await jedi.balanceOf(op.address)).sub(12000)
      );
      ...
      await expect(lockPool.unlock()).to.not.be.reverted;
      expect(await jedi.balanceOf(op.address)).to.eq(8000);
      await expect(lockPool.unlock()).to.be.revertedWith(
        "LockPool: No tokens available"
      );
      await jedi.transfer(
      ...
      await fastForwardAndMine(ethers.provider, 90 * 86400 + 1);
      await expect(lockPool.unlock()).to.not.be.reverted;
      expect(await jedi.balanceOf(op.address)).to.eq(4000);
      await expect(lockPool.unlock()).to.be.revertedWith(
        "LockPool: No tokens available"
      );
    });
  ...
  describe("#setRewardDays", () => {
    ...
    describe("#totalSupply", () => {
      it("returns the balance of all staked tokens", async () => {
        ...
        await lockPool.lock(10000, 7);
        await lockPool.lock(20000, 30);
        await jedi.transfer(other.address, 30000);
        await jedi.connect(other).approve(lockPool.address, 30000);
  ...
  describe("#migrateOwnership", () => {
    describe("when called by the Owner", () => {
      it("transfers reward token ownership and operator", async () => {
        ...
        expect(await droid.operator()).to.eq(other.address);
        expect(await droid.owner()).to.eq(other.address);
      });
    });
